...
# Transliteration pairs used by slugize.
TRANS_PAIRS = {
    'en': {
        u'æ': u'ae',
        u'é': u'e',
...
def transliterate(s, transpairs):
    ...
    Examples:
        ...
        >>> transliterate('assets', {'s': 'x'})
        u'axxetx'

        >>> transliterate('a-ssets', {r'\w': '_'})
        u'_-_____'
...
def slugify(s, locales=[], exclude_locales=[], spacer=None, max_length=0):
    ...
    ``exclude_locales`` is a list of locale codes to be excluded from conversion
    ...
    Examples:
        ...
        >>> slugify(u'\u0421\u0440\u043f\u0441\u043a\u0438') # Српски
        u'srpski'
        >>> slugify(u'\u0420\u0443\u0441\u043a\u0439') # Ruskй
        u'ruskyi'
        >>> slugify('Text with spaces')
        u'text_with_spaces'
        >>> slugify('Text with different spacer', spacer='+')
        u'text+with+different+spacer'
        >>> slugify('Punctuation, removed!')
        u'punctuation_removed'
        >>> slugify('Truncated', max_length=2)
        u'tr'
        >>> slugify(u'\u0446\u0435\u043e', locales=['sr']) # цео
        u'ceo'
        >>> slugify('no locale?!', locales=['en'], exclude_locales=['en'])
        u'no_locale'
    ...
    if not locales: locales = ['en']

    for locale in locales:
        s = transliterate(s, TRANS_PAIRS.get(locale, 'en'))
